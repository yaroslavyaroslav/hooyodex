...
fn render_linux_unit(exe: &Path, config: &Path) -> String {
    format!(
        "[Unit]\nDescription=CodexClaw Telegram Gateway\nAfter=network-online.target\nWants=network-online.target\n\n[Service]\nType=simple\nExecStart={} --config {} run\nExecReload=/bin/kill -HUP $MAINPID\nRestart=on-failure\nRestartSec=3\nTimeoutStopSec=20\nEnvironment=RUST_LOG=codexclaw=info\nEnvironment=PATH={}\nWorkingDirectory={}\n\n[Install]\nWantedBy=default.target\n",
        shell_escape(exe),
        shell_escape(config),
        service_path_env(),
        shell_escape(
...
fn render_macos_plist(exe: &Path, config: &Path) -> String {
    format!(
        ...
        service_path_env(),
        xml_escape(&default_log_dir().join("codexclaw.out.log")),
        xml_escape(&default_log_dir().join("codexclaw.err.log")),
    )
}
