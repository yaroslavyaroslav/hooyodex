---
name: hooyodex-service-restart
description: Restart, recover, and verify the local Hooyodex daemon for this repository. Use when launchd/systemd job state is ambiguous, a restart must be proven by PID change, health checks must be verified, or the local service template may need repair.
---

# Hooyodex Service Restart

Use this skill for this repository's local Hooyodex daemon only.

## What This Skill Covers

- Detect the active Hooyodex service instead of assuming a single label.
...
- Use [scripts/restart_hooyodex_service.sh](scripts/restart_hooyodex_service.sh) as the primary macOS entrypoint. It can print status, restart the active launchd job, and repair a broken plist from bundled templates.
...
- Use the concrete sanitized examples in [assets/plists/dev.hooyodex.agent.plist](assets/plists/dev.hooyodex.agent.plist), [assets/plists/com.hooyodex.agent.plist](assets/plists/com.hooyodex.agent.plist), and [assets/systemd/hooyodex.service](assets/systemd/hooyodex.service) when you need a known-good target shape.
- Use the render templates in [assets/plists/dev.hooyodex.agent.plist.template](assets/plists/dev.hooyodex.agent.plist.template) and [assets/plists/com.hooyodex.agent.plist.template](assets/plists/com.hooyodex.agent.plist.template) when the script must rewrite a local plist.
...
1. Detect the active service:
   - macOS: `scripts/restart_hooyodex_service.sh status`
